...
@pytest.mark.asyncio
async def test_workspace_mcp_generation_publish_preserves_old_turn_lease(tmp_path: Path) -> None:
    from agent.plugins.snapshot import bind_runtime_snapshot, reset_runtime_snapshot

    tools = ToolRegistry()
    manager = _manager(tmp_path, tools=tools)
    first = await manager.prepare_workspace_mcp(
        _workspace_mcp_spec(tmp_path / "workspace-v1", tool_name="version_a"),
        revision="release-a",
    )
    ...
    try:
        assert await tools.execute("mcp_workspace__version_a", {}) == "[]"
        second = await manager.prepare_workspace_mcp(
            _workspace_mcp_spec(tmp_path / "workspace-release-b", tool_name="version_b"),
            revision="release-b",
        )
        await manager.publish_workspace_mcp()
        ...
        assert await tools.execute("mcp_workspace__version_a", {}) == "[]"
        assert not tools.has_tool("mcp_workspace__version_b")
        new_lease = await manager.snapshot_store.acquire()
        new_token = bind_runtime_snapshot(new_lease)
        try:
            assert await tools.execute("mcp_workspace__version_b", {}) == "[]"
        finally:
...
@pytest.mark.asyncio
async def test_workspace_mcp_candidate_failure_keeps_active_and_cleans_partial(tmp_path: Path) -> None:
    manager = _manager(tmp_path, tools=ToolRegistry())
    first = await manager.prepare_workspace_mcp(
        _workspace_mcp_spec(tmp_path / "active", tool_name="active"), revision="active"
    )
    await manager.publish_workspace_mcp()
    ...
    specs = {"a_good": next(iter(good_spec.values())), "z_bad": {"command": [str(tmp_path / "missing-command")]}}
    with pytest.raises(FileNotFoundError):
        await manager.prepare_workspace_mcp(specs, revision="broken")
    await _wait_for_log(good_dir / "data" / "mcp-lifecycle.log", ["started", "stopped"])
    ...
    candidate = await manager.prepare_workspace_mcp(
        _workspace_mcp_spec(tmp_path / "candidate-dead", tool_name="dead"), revision="dead"
    )
