...
| C14 | inbound/outbound channel capability | `OPEN` | `fc1a2a76` 已完成 v3 plugin exact inbound/outbound、identity 持久 owner、三态 direct MessagePush、stop/drain/swap 与取消收束，定向 130 + Channel/runtime 72 tests、Pyright 0，独立 review 无 P0/P1；附件持久 owner、Core built-in transition、control/preview/stream 与 Feishu/QQ exact pair Gate 仍未闭合 | Feishu / QQBot |
