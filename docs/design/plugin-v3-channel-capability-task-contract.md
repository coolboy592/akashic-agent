...
- 状态：proposed / independent review pending
- 日期：2026-08-16
...
old stop ──► service switch ──► formal factory/start ──► provisional binding
                                                        │ finalize
                                                        ▼
                     public current = new stable ──► open admission
```

`semantic_delta: controlled`。provider parsing、attachment mapping、stream presentation 保持；本任务新增稳定
...
ChannelDefinition(
    ...
    inbound_identity=InboundIdentity.PROVIDER_MESSAGE_ID,
    credential_paths=("app_id", "client_secret"),
)
```
...
class InboundEnvelope:
    ...
    #                 binding_token, message, attachments, lease,
...
class RawInbound:
    message_id: str
    message: InboundMessage
    attachments: tuple[ImportedAttachmentLease, ...] = ()
...
class AttachmentResolver(Protocol):
    async def resolve(self, ref: AttachmentRef) -> AttachmentLease: ...

class ImportedAttachmentLease(Protocol):
    ref: AttachmentRef
    binding_token: str
    async def aclose(self) -> None: ...

class IngressAttachmentImportPort(Protocol):
    async def import_stream(
        ...
        *,
        ...
        max_bytes: int,
    ) -> ImportedAttachmentLease: ...

class CoreAttachmentImportPort(Protocol):
    async def import_core_path(
        self,
        path: Path,
        binding_lease: ChannelBindingLease,
    ) -> ImportedAttachmentLease: ...

class AttachmentLease(Protocol):
    ref: AttachmentRef
...
class ChannelControlPort(Protocol):
    async def interrupt(self, envelope: InboundEnvelope) -> ControlReceipt: ...
...
TurnStreamCallback = Callable[[TurnStreamEvent], Awaitable[None]]
...
class ChannelFactoryContext:
    ...
    attachment_import: IngressAttachmentImportPort
...
上述 `Context`、`InboundMessage`、`RuntimeSnapshotLease` 是 Core 已有 exact-runtime 类型，不由插件仓复制。
...
InboundIdentity/CredentialRef/AttachmentRef/ImportedAttachmentLease/RawInbound/ChannelAdapter` 与 factory 所需窄 Protocol。
...
1～256 非空字符并执行 bounded dedupe；不得从可变 `InboundMessage.metadata` 在后续 lane 再猜 id。现有 Feishu/QQBot
...
5. module-level static `channel_credential_paths: Mapping[channel_name, tuple[path, ...]]` 在 config 构造前由
   Core 校验；每个 Definition 的 `credential_paths` 必须与对应 static entry 完全相同。Feishu 使用
   `app_id/app_secret`，QQBot 使用 `app_id/client_secret`。candidate/formal `apply` 得到的 config 在这些路径上
...
8. `ChannelControlPort` 是 `/stop` 的唯一中断入口；`TurnStreamPort` 只投影 exact binding 的
   TurnStarted/StreamDelta/ToolCall，不暴露 EventBus。`TurnStreamCallback` 必须是 async；Host 在调用前递增 exact
   binding in-flight counter，callback terminal 的 finally 才递减。普通异常转成 owner-specific Incident 并停止该
   presentation 后续 patch；caller cancellation 先完成 callback cleanup 再传播。`StreamSubscription.close_admission()`
...
9. `AttachmentResolver` 由 Core 对 `artifact_id/sha256/size` 做完整性与归属校验，只返回 read-only bounded
   `AttachmentLease`；`AttachmentRef` 同时保留经过 basename/NUL/长度校验的 filename。入站 adapter 只能把 provider
   download stream 交给 binding-scoped `IngressAttachmentImportPort.import_stream()`，Core 在 exact binding in-flight 内边读
   边限额/hash并发布 immutable artifact，返回 `ImportedAttachmentLease`；adapter 不取得 workspace/data root。
   `RawInbound.attachments` 只能携带这些 live leases。`ChannelIngress.admit()` 原子核对 binding token并接管它们，投影
   `InboundEnvelope.attachments: tuple[AttachmentRef, ...]`；admit 前异常由 adapter finally close，admit/queue/Turn 失败由当前
   envelope owner close，Session attachment owner成功采用后才释放临时 import lease。旧 MessagePush local path 只由 Core
   tool boundary 通过不注入 factory context 的 `CoreAttachmentImportPort.import_core_path(path, ChannelBindingLease)` 导入；
   插件/adapter 看不到该方法或原始 path。随后所有 outbound 都只携带 ref，read lease 在 Receipt 后关闭。
...
6. Host 为每个 committed binding 提供 admission/in-flight counter。`ChannelIngress.admit(RawInbound)` 获取 exact
   ...
   再 acquire current；`/stop` 等 provider control command 也在同一 envelope lease 内完成并在 terminal 释放。
   旧代入队但未消费的消息因此仍用 old snapshot。
7. outbound 的 Bus 与 direct MessagePush 共享唯一 `OutboundEnvelope` owner；首次 admission 生成稳定
   ...
   `DeliveryReceipt(DELIVERED | REJECTED | UNKNOWN, delivery_id, provider_id?)`，不得用异常表达可重试结果；
   ...
   `PushToolOutboundPort.dispatch()` 只借用 lease，不得替 caller 关闭。附件 read/import leases 同样在 Receipt 后由 caller 收束。
   第一版边界固定为：
   ...
   ChannelOutboundCallback = Callable[[OutboundEnvelope], Awaitable[DeliveryReceipt]]
   async def publish_outbound_awaited(envelope: OutboundEnvelope) -> DeliveryReceipt: ...
...
8. `OutboundEnvelope` 固定 `logical_delivery_id/delivery_id/attempt_sequence/snapshot_id/generation_id/binding_token/
   channel/recipient/body/attachments/metadata`；队列和订阅按 exact binding token 路由，不能只按 channel name。
...
9. `MessageBus.publish_outbound_awaited()`、Bus callback、`BusOutboundPort.dispatch()` 与 direct Push 都返回同一个
   `DeliveryReceipt`。AgentLoop 的正常 assistant、异常/取消 terminal、provider control response 与所有其他用户可见
...
11. inbound envelope 构造后先处于 `INGRESS/ADMITTED`，随后固定四个处理态
    ...
    清空 `_lane_queues`。
12. `/stop` 等 provider control 命令先在其 inbound envelope lease 内执行 Core control，再通过同一 committed
    outbound binding 的 `BusOutboundPort.dispatch()` 发送响应；禁止插件直接 `self.send()`。旧 binding admission
    已关闭时返回明确 `REJECTED`，provider 调用后失败仍是 `UNKNOWN`。
13. 普通 assistant、error、cancel 与 control response 全部从发起 Turn 的 `InboundEnvelope` 派生 outbound identity；
    `snapshot_id/generation_id/binding_token` 必须继承 exact inbound lease，不能在 Turn terminal 时按 channel name 或
    current snapshot 重查。无 inbound 的 proactive/direct Push 必须从其调用时持有的 exact snapshot lease，经
...
最后一个 v2
channel 迁走并通过 E3 后，Host 只消费 committed registry，删除 `Plugin.channels()`、
...
v3/v2 collision fail-loud，不允许覆盖。迁移期 v2 callback 用薄 adapter 转成 `DeliveryReceipt`：正常返回只能在
...
- credential：candidate/formal apply 只见 CredentialRef；candidate provider 调用为 0；正式 config/plugin-data digest 不变；
...
- attachment：hash/size/owner 不匹配 fail-loud，read-only AttachmentLease 在 Receipt 后归零，adapter 无 workspace path；
...
outbound(delivery_id, body, attachments)
→ logical_attempt_count(logical_delivery_id)=1
→ raw_effect(part_index, chunk_or_attachment, same_delivery_id)*
...
→ awaited DeliveryReceipt before Turn terminal
...
一个 logical delivery 可产生多个 chunk/attachment raw effects，但全部共享 delivery id；任一已可能提交的 part 失败，
...
1. C14a：typed Definition/Registry、CredentialRef projection、Root provider、snapshot identity 与 collision。
2. C14b：`CommittedChannelCatalog`、ChannelHost closed provisional binding、binding-token queue/in-flight lease、
   critical cleanup、delivery outcome。stop/close 失败保留 cleanup-pending binding、聚合全部错误并提供显式 retry；
   成功前不能移除 owner/closeables。
3. Feishu、QQBot canonical v3 PR：领域单测 + Manager formal-generation test，不做插件级完整 E2E。
4. exact pair E3 后删除 v2 channel public ABI/fixed contribution/live snapshot path。
