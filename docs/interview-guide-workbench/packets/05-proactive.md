...
解释一条外部内容从进入系统到被发送或跳过的实际过程。用户没有发起当前请求时，系统还要判断兴趣、依据和打扰时机；主动与被动都可能产生外部效果，不能把被动风险一概说成只影响回复。重点讲清触发、两阶段模型决策和发送处理，不只罗列状态边界。
...
- 外部内容由什么入口进入，来源与 revision 如何形成；相关能力在插件时，沿一个对应版本的真实接入场景解释，不扩写全部插件。
- 兴趣信息来自哪里，哪些判断由程序完成、哪些需要模型；衰减池和新 revision 条件为什么同时存在，阈值参数如何影响触发而非宣称效果最优。
- 初筛具体看到什么、输出什么待确认问题，调查怎样用记忆或外部内容核对；为什么不一次完成，漏筛与调查成本怎样权衡。
- 兴趣相关、事实有依据、内容有新意、当前适合打扰不是同一个判断。按实际实现解释各自如何处理，不声称系统已有完全可靠的主题去重。
- 合法 share 到真实送达还经历什么，skip 与投递未知怎样处理；状态术语服务于这个具体问题，不用状态清单取代设计解释。
