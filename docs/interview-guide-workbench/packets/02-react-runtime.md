...
用一次需要工具的请求解释历史完整基线中的共性执行方式，并说明为什么记忆、工具发现、主动交互和 Terminal-Bench 能建立在同一运行时思路上。

本章是地基，目标是恢复对本项目请求组装、模型与工具交互、完成及中断处理的具体理解。状态与边界服务于机制解释，不把它扩展成第五项简历成绩，不再提供独立口述骨架。
...
- 沿一次工具请求说明模型具体收到什么、工具如何被选中和执行、调用结果怎样回到后续请求；不满足于“判断—行动—观察”的通用循环。
- 正常完成、工具出错、用户中断和模型容量错误各会影响哪一步，哪些已经发生的外部效果不能被取消；按问题需要选择分支，不逐项罗列内部状态。
- 主动任务和 benchmark 与普通对话在哪些入口、工具约束和终态上不同，复用的究竟是什么。
- 需要解释插件扩展时，沿对应版本的注册或调用关系阅读相关实现，不预设主仓库已覆盖全部能力，也不因此扫描整个插件组织。
